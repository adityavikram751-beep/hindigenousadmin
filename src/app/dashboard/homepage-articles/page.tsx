...
import { homeArticleApi } from '@/lib/api';
...
export default function HomepageArticlesPage() {
  ...
  const fetchArticles = async () => {
    ...
    try {
      ...
      if (authorFilter.trim()) {
        res = await homeArticleApi.getByAuthor(authorFilter.trim());
      } else {
        res = await homeArticleApi.getAll();
      }
      const list = Array.isArray(res) ? res : res?.data || res?.articles || [];
      setArticles(list);
    } catch {
      showToast('Error', 'Failed to fetch homepage articles', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (formData: FormData) => {
    if (editingItem) {
      await homeArticleApi.update(editingItem._id || editingItem.id, formData);
      showToast('Updated', 'Homepage article updated successfully', 'success');
    } else {
      await homeArticleApi.create(formData);
      showToast('Published', 'New homepage article created successfully', 'success');
    }
    fetchArticles();
  };
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await homeArticleApi.delete(id);
      showToast('Deleted', 'Homepage article removed', 'info');
      fetchArticles();
    } catch {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-[#1E2B45]">
        <div>
          <h1 className="text-2xl font-extrabold text-white tracking-tight flex items-center gap-2.5">
            ...
            <span>Homepage Articles CMS</span>
          </h1>
          <p className="text-xs text-slate-400 mt-1">
            Manage featured stories, cultural blogs, and main page article entries
          </p>
        </div>
      ...
      <div className="bg-[#131C2E] rounded-2xl border border-[#23314D] overflow-hidden">
        {loading ? (
          <div className="p-12 text-center text-slate-400 flex flex-col items-center space-y-3">
            ...
            <p className="text-sm font-medium">Fetching homepage articles...</p>
          </div>
        ) : filteredArticles.length === 0 ? (
          <div className="p-12 text-center text-slate-400 space-y-3">
            <FileText className="w-12 h-12 text-slate-600 mx-auto" />
            <h3 className="text-lg font-bold text-white">No Homepage Articles Found</h3>
            <p className="text-xs max-w-sm mx-auto">
              No articles found matching your criteria. Click below to create a new article.
            </p>
            <button
              onClick={() => {
                setEditingItem(null);
                  ...
                  <tr key={art._id || art.id} className="hover:bg-[#162138]/70 transition-colors">
                    ...
                    <td className="py-3.5 px-5 text-xs text-slate-300">
                      <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-indigo-500/10 text-indigo-300 border border-indigo-500/20 font-semibold">
                        ...
                        {art.authorName || 'Hindigenous Editorial'}
                      </span>
                    </td>
      ...
      <ArticleFormModal
        ...
        title={editingItem ? 'Edit Homepage Article' : 'Publish New Homepage Article'}
      ...
      {viewingItem && (
        <Modal
          ...
          subtitle={`By ${viewingItem.authorName || 'Author'}`}
          maxWidth="max-w-2xl"
        >
