...
export const Sidebar = ({ onOpenSettings, isOpen = false, onClose }: SidebarProps) => {
  ...
  return (
    <>
      ...
      {/* Sidebar Panel */}
      <aside
        className={`w-64 h-screen bg-[#0B0F19] border-r border-[#1E293B] flex flex-col fixed left-0 top-0 z-50 transition-transform duration-200 lg:translate-x-0 ${
          isOpen ? 'translate-x-0' : '-translate-x-full'
        } select-none`}
      >
        {/* Brand Header */}
        <div className="p-4 border-b border-[#1E293B] flex items-center justify-between">
          <Link href="/dashboard" onClick={handleNavClick} className="flex items-center space-x-2.5 group">
          ...
          {onClose && (
            <button
              onClick={onClose}
              className="lg:hidden p-1.5 text-slate-400 hover:text-white rounded-lg hover:bg-[#1E293B]"
            >
              <X className="w-5 h-5" />
            </button>
          )}
        ...
        <div className="p-3 border-t border-[#1E293B] space-y-2 bg-[#080C14]">
          <button
            ...
            className="w-full flex items-center justify-between px-3 py-2 rounded-lg text-xs text-slate-300 hover:bg-[#1E293B] transition-colors border border-[#1E293B]"
          >
